...
const primaryColors = [
  ...
  { token: "--cpsl-blue-500", hex: "#4A78E8", name: "Blue 500", main: true },
...
const accentColors = [
  ...
  { token: "--cpsl-purple-500", hex: "#8B40D4", name: "Purple 500", main: true },
...
const goldColors = [
  ...
  { token: "--cpsl-gold-500", hex: "#BFA05A", name: "Gold 500", main: true },
...
const contrastRows = [
  ["White on Blue-500 (#4A78E8)",       "3.7:1",  "❌", "✅", "❌"],
  ["Dark (#0A0E1A) on Blue-500",        "5.2:1",  "✅", "✅", "❌"],
  ["White on Dark (#0A0E1A)",           "18.9:1", "✅", "✅", "✅"],
  ["N-900 on N-50",                     "14.2:1", "✅", "✅", "✅"],
  ["N-600 on White",                    "5.74:1", "✅", "✅", "❌"],
  ["Dark text on Gold-500 (#BFA05A)",   "8.0:1",  "✅", "✅", "✅"],
  ["White on Purple-500 (#8B40D4)",     "6.7:1",  "✅", "✅", "❌"],
  ...
  return (
    <div style={{ background: "#F4F6FA", minHeight: "100vh" }}>
      <PageHeader section="01 — Foundations" title="Color System"
        description="CPSL Blue (#4A78E8), Gold (#BFA05A), and Purple (#8B40D4) anchor the new brand palette. Cream (#F5EFE0) adds warmth to premium surfaces. All colors ship as CSS custom properties and design token JSON." />
      <div className="px-12 py-12">
        ...
        <Section title="Championship — CPSL Gold">
          ...
          <div className="flex rounded-2xl overflow-hidden h-20 mb-4 shadow-sm">
            {goldColors.map((c) => (
              <div key={c.hex} className="flex-1 relative" style={{ background: c.hex }}>
                {c.main && <div className="absolute inset-0 flex items-end p-2"><span className="text-xs font-bold px-1.5 py-0.5 rounded bg-black/10" style={{ color: "#0A0E1A" }}>Gold</span></div>}
              </div>
            ))}
          </div>
            ...
            {accentColors.map((c) => (
              <div key={c.hex} className="rounded-xl overflow-hidden border flex-1" style={{ borderColor: "#E2E8F0" }}>
                ...
                <div className="p-3" style={{ background: "white" }}>
                  <div className="text-xs font-bold" style={{ color: "#0A0E1A" }}>{c.name}</div>
                  <div className="text-xs font-mono" style={{ color: "#64748B" }}>{c.hex}</div>
                  {c.main && <div className="text-xs mt-1 font-semibold" style={{ color: "#8B40D4" }}>Accent</div>}
                </div>
              </div>
            ))}
          </div>
        ...
        <Section title="Warm Surface — CPSL Cream">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              { token: "--cpsl-cream",      hex: "#F5EFE0", name: "Cream",      label: "Premium backgrounds, feature panels" },
        ...
        <Section title="WCAG Contrast Ratios">
          <p className="text-xs text-muted-foreground mb-4">
            The new primary blue (#4A78E8) passes AA for <strong>large text</strong> with white. For small body copy on a blue button, use dark text — it achieves 5.2:1 (AA Normal). Gold (#BFA05A) always requires dark text.
          </p>
        ...
        <Section title="Dark Mode Tokens">
          <div className="rounded-2xl p-6 border" style={{ background: "#020B1A", borderColor: "#1E2D45" }}>
            <div className="grid grid-cols-4 gap-4">
              {[
                { token: "--primary",      light: "#4A78E8", dark: "#6A9AF2"  },
                { token: "--accent",       light: "#8B40D4", dark: "#B08CF5"  },
                { token: "--cpsl-gold",    light: "#BFA05A", dark: "#CDB268"  },
                { token: "--surface-warm", light: "#F5EFE0", dark: "#1A1208"  },
